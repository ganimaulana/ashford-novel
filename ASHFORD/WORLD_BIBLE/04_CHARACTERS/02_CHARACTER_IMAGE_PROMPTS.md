# CHARACTER IMAGE PROMPTS — Alderwick
...
>
> Source hierarchy: manuscript → WORLD_BIBLE → `05_CHARACTER_VISUAL_CANON.md` → `01_CHARACTER_IMAGE_GENERATION_GUIDE.md` → these prompts.
...
Each prompt below is written in a mode-neutral form; when generating, declare the mode by adding the appropriate style block from `01_CHARACTER_IMAGE_GENERATION_GUIDE.md` (Section III, Dual Visual Mode System).
...
no anime/cartoon appearance [REALISM MODE ONLY; in ANIME MODE use mature seinen-style anime aesthetic from 01_CHARACTER_IMAGE_GENERATION_GUIDE.md §III]
...
- **Dual-mode:** Each prompt may be rendered in ANIME or REALISM mode. The mode alters rendering style only — never identity, age, clothing period, or canon. See `01_CHARACTER_IMAGE_GENERATION_GUIDE.md` §III (Dual Visual Mode System) for mode style blocks and per-mode negative constraints.
