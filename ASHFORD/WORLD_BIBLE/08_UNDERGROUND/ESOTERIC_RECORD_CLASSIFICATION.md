...
- "ARC-001"… as supernatural-object designations → deprecated → superseded by **ERC-001**…
...
To provide Ashford with a historical, esoteric, antiquarian designation system for unexplained subjects — discovered through old documents and investigation, never through a modern database.
...
The existing `07_ANOMALIES/Classification_Guide.md` categories (OBJECT, LOCATION, PERSON, EVENT, PHENOMENON, ENTITY, DOCUMENT) are the historical predecessor; ERC refines them. R0–R5/NULL risk tiers may be retained as author-side aids only — never as SCP-style containment classes in prose.
...
- Migration mapping: former development designations carry over numerically — ARC-008 (The Returning Coin) → **ERC-008**; ARC-013 (The Thirteenth Key) → **ERC-013**; ARC-021 (The Bellweather Clock) → **ERC-021**; ARC-001…ARC-020 index entries → ERC-001…ERC-020 (all PROPOSED/development entries).
...
- Some ERC records may later be proven mundane, misunderstood, incomplete, or incorrectly classified. This ambiguity is essential to Ashford's mystery tone.
...
Gear system reference: `DOOR_GEAR_SYSTEM.md` and `GEAR_COMPONENT_PROFILES.md`. Gear count remains **UNRESOLVED / TO BE DETERMINED** (nine is PROPOSED, not confirmed). Complete assembly consequence: **UNKNOWN / INTENTIONALLY UNRESOLVED**.
